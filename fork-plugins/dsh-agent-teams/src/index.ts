...
export interface Config {
  ...
  /** Prompt-section order for the usage policy (default: immediately after first-party team policy). */
...
export function apply(ctx: Context, config: Config): void {
  ...
  ctx.systemPrompt.section({
    ...
    order: config.promptSectionOrder ?? ctx.systemPrompt.getSectionOrder('TEAM_POLICY') + 10,
