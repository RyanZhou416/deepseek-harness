...
export function validatePolicy(input) {
  if (input.schemaVersion !== 1 || !/^(?:next|dsh[a-z0-9]+)$/.test(input.previewTag)) {
    throw new Error('Unsupported compatibility policy')
  }
  ...
  if (/-(alpha|beta)\./.test(input.recommendedHost) && input.previewTag === 'next') {
    throw new Error('Alpha/beta hosts cannot be the ordinary-user recommendation')
  }
