...
const npmCommand = process.platform === 'win32'
    ? [process.execPath, existsSync(process.env.npm_execpath ?? '') ? process.env.npm_execpath : bundledNpmCli]
    : ['npm'];
if (process.platform === 'win32' && !existsSync(npmCommand[1]))
    throw Error('Cannot locate npm-cli.js beside Node; run this verifier from an npm-managed environment');
const isDsh = name => /^@deepseek-ai\/dsh(?:-|$)/.test(name);
const json = (path, value) => writeFileSync(path, JSON.stringify(value, null, 2) + '\n');
const hash = path => createHash('sha256').update(readFileSync(path)).digest('hex');
const environment = (extra = {}) => ({ PATH: process.env.PATH, LANG: process.env.LANG ?? 'en_US.UTF-8', HOME: join(report, 'user-home'), TMPDIR: join(report, 'tmp'), npm_config_userconfig: nullDevice, npm_config_registry: registry, ...extra });
...
const install = await command([...npmCommand, 'install', '--prefer-online', '--no-audit', '--no-fund', '--registry=' + registry, '--userconfig=' + nullDevice], runtime, environment({ 'npm_config_cache': process.env.npm_config_cache ?? join(report, 'npm-cache') }), 'install');
if (install.code !== 0)
...
for (const scenario of (flags.has('--scenario') ? [flags.get('--scenario')] : scenarios)) {
    ...
    symlinkSync(join(runtime, 'node_modules/@nanmicoder/dsh-agent-teams'), join(profile, 'node_modules/@nanmicoder/dsh-agent-teams'), process.platform === 'win32' ? 'junction' : 'dir');
