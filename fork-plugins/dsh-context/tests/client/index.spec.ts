// DeepSeek Harness fork modification: cover the 0.1.6 settingsScope/Plugins seat pairing.
...
describe('client entry: settingsScope inject', () => {
  ...
  test('armed later: the pending inject runs — scope attached and card slot registered', () => {
    ...
    assert.equal(ctx.slots.of('plugins.bundle.config').length, 1)
    ctx.dispose()
  })

  test('the 0.1.6 Plugins seat renders through settingsScope', async () => {
    const scope = makeScope({ status: 'ready', value: {}, writable: true })
    const ctx = new TestClientCtx({ services: { settingsScope: { bind: () => scope } } })
    try {
      applyTo(ctx)
      const entry = ctx.slots.of('plugins.bundle.config')[0]
      assert.equal(entry.registration.key, 'dsh-context')
      const face = entry.registration.inject?.() as { hooks: { contextSettings: { getSnapshot(): SettingsState } } }
      const element = entry.component({}) as ReactElement
      const mounted = await mount(h(element.type as never, {
        useContextSettings: <T,>(select: (state: SettingsState) => T): T => select(face.hooks.contextSettings.getSnapshot()),
      }))
      try {
        assert.equal(queryAll(mounted.container, '.lc-settings-select').length, 6)
      } finally {
