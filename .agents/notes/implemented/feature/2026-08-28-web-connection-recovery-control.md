...
**Render a full-width `ConnectionBanner` at the top of the viewport.** Rejected because a full-width strip consumes unrelated page chrome. The global presentation uses the same compact, actionable `ConnectionIndicator` as Settings and occupies only its top-center footprint; no `ConnectionBanner` compatibility export exists before the first tagged release.
