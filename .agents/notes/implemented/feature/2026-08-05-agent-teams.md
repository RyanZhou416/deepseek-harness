...
For teammate-originated mail, quiet `send_message` reaches a live target without waking it, while an inactive target remains queued until another event materializes that teammate; waking `followup_task` becomes the target's next FIFO turn and may cold-resume it. Lead-to-teammate mail instead follows the next-step policy owned by the [steer-responsive Team work decision](2026-08-30-steer-responsive-agent-team-work.md). Success means the message is already durable even when immediate delivery is deferred. The mechanism provides process-local retry and target-Session de-duplication, not a cross-process exactly-once claim.
