...
如果在一组启动前中止，系统不会记录该组的任何调用。如果在一组执行期间中止，系统会停止补充池，等待已启动的调用，按顺序提交其结果，在这些结果之后排空已接受的批次上下文，然后通过现有中止路径结束该步骤。从未启动的调用没有审计事件。调度器发生意外故障时，会停止新的派发并等待每项已启动的派发结算。随后，它会为工具主体可能已启动的调用按序记录 `TOOL_OUTCOME_UNKNOWN` 结果，为其余调用记录 `TOOL_NOT_STARTED` 结果，再重新抛出第一个故障（[故障结算](../bug-fix/2026-09-18-tool-scheduler-failure-settlement.zh.md)）。
...
**让调度器故障不写工具结果。** 这是最初选择，因为 AgentLoop 无法确定被拒绝的派发是否调用了工具主体。提供方有效历史后来要求显式结果，因此该选择已被取代：循环现在区分可能已进入派发的调用和确定未进入派发的调用，但不会把任一情况重新分类为 `ABORTED`（[决策](../bug-fix/2026-09-18-tool-scheduler-failure-settlement.zh.md)）。
...
终态调度器故障会等待仍在运行的派发，再在失败 step 结束前用显式 unknown/not-started 结果关闭每项模型调用。结果记录执行确定性，而不把内部故障表述为普通工具结果。
