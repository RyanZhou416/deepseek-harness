...
An abort before a group starts records no calls from that group. An abort during a group stops replenishment, waits for already-started calls, commits their results in order, drains accepted batch context after those results, and then ends the step through the existing abort path. Calls that never start have no audit event. An unexpected scheduler failure stops new dispatches and waits for every already-started dispatch to settle. It then records ordered `TOOL_OUTCOME_UNKNOWN` results for calls whose bodies may have started and `TOOL_NOT_STARTED` results for the rest before rethrowing the first failure ([failure settlement](../bug-fix/2026-09-18-tool-scheduler-failure-settlement.md)).
...
**Leave scheduler failures without tool results.** This was the original choice because AgentLoop cannot know whether a rejected dispatch invoked the tool body. It was superseded when provider-valid history required explicit results: the loop now distinguishes calls that may have entered dispatch from calls known not to have entered it, without reclassifying either as `ABORTED` ([decision](../bug-fix/2026-09-18-tool-scheduler-failure-settlement.md)).
...
A terminal scheduler failure waits for live dispatches, then closes every model call with an explicit unknown/not-started result before the failed step ends. The result records execution certainty without presenting the internal failure as an ordinary tool outcome.
