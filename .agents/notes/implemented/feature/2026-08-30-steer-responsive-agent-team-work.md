...
The continuable Subagent service exposes `steer(parent, childId, content, options)` beside FIFO `followup()`. Both operations retain exact-live-parent authorization, per-child serialization, cold resume, Activation ownership, and inbox-acceptance cancellation. Steering calls the child Agent's existing `steer()` operation, so a running child claims the message at its nearest step boundary and an idle child wakes.

Agent Teams routes every Lead-to-teammate mailbox delivery through Subagent steering regardless of the model-requested quiet or wakeup mode. Teammate-originated peer messages retain the durable quiet and next-turn modes from the [Agent Teams decision](2026-08-05-agent-teams.md). The Team service derives this policy from exact membership and sender identity; prompts and tool arguments do not enforce it.

The global model-facing `send_message` control also calls Subagent steering, so ordinary coordinator-to-child instructions use the same nearest-step behavior outside Agent Teams. Host-user and browser prompt routes retain FIFO follow-up delivery; changing the model tool does not reclassify human input.

The browser Queue Dock exposes the existing steering action for queued messages in a running continuable-child conversation. The Client routes that action through a dedicated `subagents/steerQueuedByParent` Remote, which authorizes the durable direct-parent address and promotes only the selected pending next-turn occurrence. Edit and remove remain unavailable for subagent queues.

The Bash, PowerShell, and persistent-terminal tool consumers accept `forceRunInBackground`, default `false`. An enabled consumer hides `run_in_background`, requires background support and `ctx.jobs` at load, and registers every command or terminal send as an owner-scoped job before returning its id. The process remains under the job runtime until completion, cancellation, owner disposal, or service disposal.
...
Subagent tests distinguish next-step steering from FIFO follow-up for a running Activation and retain the existing authorization, cold-resume, disposal, and one-shot behaviors. Queue Dock and Client tests cover the continuable-child button, one-shot read-only state, durable address routing, stale-occurrence convergence, and hidden edit/remove actions; a Web end-to-end case exercises the real Remote and resumed inbox order. Team mailbox tests cover Lead steering, teammate quiet/FIFO delivery, target-local serialization, persistence recovery, interruption, and pending limits. Job tests prove next-step yielding leaves the job running, next-turn messages do not yield, and the default remains unchanged. Bash, PowerShell, terminal, and profile tests prove forced jobs remove the model argument, return real job ids, present background results, and fail at load without required capabilities.

## Consequences

A Lead directive still waits for the child's current model request or tool call to finish; steering does not cancel a step. Forced background execution prevents shell and terminal work from owning that step, while steer-responsive `job_output` prevents a later passive job wait from extending it after next-step input arrives.
...
The generic background and wait options add no behavior or schema change at their false defaults. The global model-facing `send_message` tool deliberately changes from later-turn FIFO to nearest-step steering. The private Team profile changes shell calls to return job ids and lets `job_output(wait: true)` return `[status: running]` before its timeout when next-step input is pending.
