...
The browser Queue Dock exposes the existing steering action for queued messages in a running continuable-child conversation. The Client routes that action through a dedicated `subagents/steerQueuedByParent` Remote, which authorizes the durable direct-parent address and promotes only the selected pending next-turn occurrence. The Remote reports stale and inactive targets through the shared namespaced `RemoteError` codes `subagent/queue-item-not-found` and `subagent/steer-unavailable`. Edit and remove remain unavailable for subagent queues.
