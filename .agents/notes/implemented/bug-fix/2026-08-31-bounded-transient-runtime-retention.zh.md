...
可续传 subagent 保持既有生命周期：continuation manager 已经会立即释放 settled Activation，并在后续投递时冷恢复。这里不增加外部 idle timer，也不增加 Agent 并发限制。
...
**依赖 watchdog 重启。** 拒绝将其作为稳态机制，因为重启只有在中断服务后才能释放整个 heap。它继续作为紧急后备。
