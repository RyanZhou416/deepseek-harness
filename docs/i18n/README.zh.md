...
- `fork-plugins/dsh-agent-teams/`、`fork-plugins/dsh-context/` 与 `fork-plugins/dsh-plugin-subscriptions/`：精确导入的外部源码保留上游文档、发行证据与原生语言政策；由 DSH 维护的整合说明仍位于双语配对的 [`fork-plugins/README.md`](../../fork-plugins/README.zh.md) 和活跃 Agent Note 中。
