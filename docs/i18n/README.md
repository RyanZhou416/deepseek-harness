...
- `fork-plugins/dsh-agent-teams/`, `fork-plugins/dsh-context/`, and `fork-plugins/dsh-plugin-subscriptions/` — exact external source imports retain their upstream documentation, release evidence, and native language policy; DSH-owned integration guidance remains in the paired [`fork-plugins/README.md`](../../fork-plugins/README.md) and active Agent Notes.
