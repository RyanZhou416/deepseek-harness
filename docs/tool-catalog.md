...
Inspect one previously accepted Session message without waking the target. Use this when the user needs delivery diagnosis or before deciding on recovery; do not poll or call it repeatedly. The result distinguishes pending Queue state, claim before model admission, model context, unresolved foreground tools (including terminal_send), completed turns, rejected steps, durable cancellation, and an unknown target/message pair.
