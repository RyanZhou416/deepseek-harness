...
- `@deepseek-ai/dsh-tool-session-message` — 需要 `tools` · `agents` · `sessionController` · `sessionReferenceResolver` · `sessionQuery` · `permissionPresets`（[`packages/api/tool-session-message/src/index.ts`](../packages/api/tool-session-message/src/index.ts)）
