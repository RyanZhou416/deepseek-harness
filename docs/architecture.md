...
One inbox feeds the driver. Wake messages start it; idle injections wait. AgentLoop's durable `inbox` projection exposes pending input without live Agents. Input admitted after the running driver's final inbox decision latches a replacement turn after convergence.
