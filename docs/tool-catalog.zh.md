...
在不唤醒目标的情况下检查一条此前已接受的 Session 消息。当用户需要投递诊断或决定恢复措施前使用本工具；不要轮询或重复调用。结果会区分待处理注入上下文或旧版排队轮次、模型准入前的领取、模型上下文、未结算前台工具（包括 terminal_send）、已完成轮次、被拒绝步骤、持久取消与未知目标／消息对。
