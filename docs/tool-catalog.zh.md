...
根据 subagent id 向后台 subagent 发送消息，继续同一段对话。该消息会加入 subagent 最近的 step 边界：如果它仍在工作，当前模型请求或工具调用会先结束，随后消息可以重定向剩余工作。此调用不会返回 subagent 的答案，只会确认消息已投递，因此请用它分派更多工作。调用失败表示消息**未**投递。
...
向 teammate 发送一条在其最近 step 边界投递的持久指令。
