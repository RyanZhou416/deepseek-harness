...
<a id="deepseek-aidsh-tool-session-message"></a>
...
在不唤醒目标的情况下检查一条此前已接受的 Session 消息。当用户需要投递诊断或决定恢复措施前使用本工具；不要轮询或重复调用。结果会区分待处理 Queue 状态、模型准入前的领取、模型上下文、未结算前台工具（包括 terminal_send）、已完成轮次、被拒绝步骤、持久取消与未知目标／消息对。
...
Web bundle 公开按标题引导的根 Session 发现、无限制确切 id FIFO 投递与只读状态检查。发送者来源取自确切调用 Agent；系统使用提示词指导而非 runtime 目标或频率策略来保留专用消息路径，并阻止循环与轮询。
