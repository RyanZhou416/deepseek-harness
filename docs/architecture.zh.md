...
[Cordis](cordis-primer.zh.md) 支撑 dsh：插件向共享上下文贡献服务、类型化事件和可逆的副作用。模型适配器、工具注册表、会话日志及 agent loop（智能体循环）也都是可从配置替换的插件。
...
inbox 驱动工作：唤醒消息会启动它；idle 注入保持等待。其持久投影使待处理输入在没有 Agent 时仍可读取。最终 inbox 决定后的输入会锁存一个收敛后替代轮次。
