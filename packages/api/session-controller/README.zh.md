...
| 字段 | 默认值 | 含义 |
|---|---:|---|
| `idleSessionRetentionMs` | `300000` | Controller 自己持有、已持久化的空闲 Agent 保持驻留的毫秒数；`0` 禁用淘汰 |
...
Controller 自己持有的 Agent 在空闲且没有待处理收件箱输入、存活子 Agent 或运行中／停止中的任务时可被淘汰。历史 follower 仅在开场帧交付前保留 Agent；之后它释放完整 Session 观察，同时保持事件监听器工作。到期后，Controller 会刷新 Session、通过持久层确认它，再只释放自己持有的实时句柄；Session 列表项、已打开的 follow 和持久历史仍然保留，下一个会激活 Session 的操作会冷恢复它。没有持久层时，Agent 保持驻留。
