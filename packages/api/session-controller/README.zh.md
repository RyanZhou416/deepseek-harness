...
控制器会为它创建或恢复的每个普通 Session 保留 `AgentHandle`。一个已归属 Agent 若持续 idle、没有 follower、待处理 inbox 输入、live child 或活跃 job，且拥有已持久刷新的快照，就会在 `idleSessionRetentionMs` 后卸载；其列表行与存储仍可用于冷恢复。默认保留时间为五分钟，`0` 禁用回收。
