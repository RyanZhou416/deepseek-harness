...
Controller 为它创建或恢复的每个普通 Session 保留 `AgentHandle`。当自有 Agent 持续 idle，且没有 follower、pending inbox input、live child 或 active job，并已有持久化 flush snapshot 时，会在 `idleSessionRetentionMs` 后卸载；其列表行与存储仍可用于冷恢复。默认保留五分钟，设为 `0` 可禁用淘汰。
...
Client 导航只为当前 staged Session 保持 history Remote。切换 stage 或清除选择会在 former stream 达到 quiescence 后将其挂起，同时保留 Session 对象、已加载窗口、projection、queue、draft 与 scoped feature state；返回该 Session 时会从持久 Host 历史替换保留窗口。
...
| 字段 | 默认值 | 含义 |
|---|---:|---|
| `idleSessionRetentionMs` | `300000` | 自有持久 Agent 卸载前的 idle 间隔；`0` 禁用淘汰 |
