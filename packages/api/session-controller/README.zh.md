...
本 controller 创建或恢复的 Agent 具有有界的空闲驻留时间。历史仍有 follower、inbox 或自有子 Agent 仍有工作、或其 job 仍在运行时，空闲 Agent 会继续驻留。若持续 `idleSessionRetentionMs` 都不存在这些所有者，controller 会 flush Session、确认存在持久产物，并且只释放自己持有的 `AgentHandle`；持久化列表行继续可见，后续操作可正常恢复它。

Client adapter 提供 `SessionEventStream`，即绑定到一个普通 Session 或 direct subagent address 的 Gateway `RemoteJournalStream`。它在读取首个 page 前打开 follow，只发布连续的 `replace`、`prepend` 和 `append` 变更，并通过 tail page 修复重连或 seq 缺口。普通 record 覆盖 `[event.seq, event.seq]`，packed row 覆盖 `[event.seq, event.seq + memberCount - 1]`。累计 20,000 个 scalar live append 后，下一条最终 `assistant/message` 会让 stream 重启一次；其 packed opening snapshot 会替换累积的 scalar tail 并重置计数。只有 staged Session 保持 history follow 打开：选择其他 Session 或显式清空选择会摘掉上一条 stream，同时保留它的 scope 与当前 window；再次选择时从持久历史重建。临时 list masked gap 会继续保留 stage，因为它属于 transport 状态而不是用户导航。业务、persistence 或无法恢复的连续性错误会终止 stream，只有物理载体断开才触发自动恢复。`SessionControlStream` 是 Gateway `RemoteSnapshotStream`；每代都以完整的进程本地 baseline 开始，因此重连会替换 queue、jobs 和 projection 状态，而不会把瞬态值当作 durable event。
...
| `idleSessionRetentionMs` | `300,000` | 本 controller 拥有的持久化、无 follower Agent 的空闲驻留时间；`0` 禁用逐出 |
