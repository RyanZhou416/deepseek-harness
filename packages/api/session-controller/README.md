...
The controller retains the `AgentHandle` for every ordinary Session it creates or resumes. An owned Agent that stays idle, has no follower, pending inbox input, live child, or active job, and has a durable flushed snapshot unloads after `idleSessionRetentionMs`; its list row and storage remain available for cold resume. The default retention is five minutes, and `0` disables eviction.
...
Client navigation keeps a history Remote only for the staged Session. Moving the stage or clearing the selection suspends the former stream after it reaches quiescence while preserving the Session object, loaded window, projections, queue, drafts, and scoped feature state; returning to that Session replaces the retained window from durable Host history.
...
|---|---:|---|
| `idleSessionRetentionMs` | `300000` | Idle interval before an owned durable Agent unloads; `0` disables eviction |
