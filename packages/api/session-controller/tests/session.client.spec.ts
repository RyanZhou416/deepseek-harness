...
describe('live event path', () => {
  ...
  it('restarts for a packed rebaseline only after the live threshold reaches a final message', async () => {
    const restart = vi.spyOn(SessionEventStream.prototype, 'restart')
    const api = new FakeApiClient()
    const session = new Session(SID, fakeRemote(api), { liveEventRebaseThreshold: 2 })
    const source = plainTurn(0, 0, 'a', 'b')
    api.onHistory = () => histResponse(source)
    await session.open()

    const turn = ev.turnStart(6, 1)
    source.push(turn)
    await follow(api, turn)
    const user = ev.user(7, 'next')
    source.push(user)
    await follow(api, user)

    expect(api.followStarts).toEqual([SID])

    const final = ev.assistant(8, 1, 'done')
    source.push(final)
    await follow(api, final)
    expect(restart).toHaveBeenCalledOnce()
    await vi.waitFor(() => { expect(api.followStarts).toEqual([SID, SID]) })
    await vi.waitFor(() => {
      expect(session.eventSource.getSnapshot().change).toMatchObject({ kind: 'replace' })
    })
    expect(eventSeqs(session)).toEqual(source.map(event => event.seq))
  })
})
