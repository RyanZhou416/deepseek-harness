...
`createApiRemoteAgentResolver()` 会复用 live Agent、恢复普通冷会话、对并发恢复去重、保留 subagent ownership fence，并为 Typert `agent` 和 `session` lookup 配置同一个 resolver。可选的 `ownHandle` 回调会把精确的 resumed `AgentHandle` 转交给 Host residency 策略；该回调拒绝所有权时，resolver 会先 dispose handle，再返回 internal failure。标准 Web API Proxy 提供 Agent 默认值、scope 设置与 handle owner，再将返回的 resolver 用于旧方法，使已迁移与未迁移方法共用同一份策略实现。
