...
`createApiRemoteAgentResolver()` reuses live Agents, resumes ordinary cold sessions, deduplicates concurrent resumes, preserves the subagent ownership fence, and configures the same resolver for Typert `agent` and `session` lookups. Its optional `ownHandle` callback transfers the exact resumed `AgentHandle` to a Host residency policy; if that callback rejects ownership, the resolver disposes the handle before returning an internal failure. The standard Web API Proxy supplies its Agent defaults, scope setup, and handle owner, then uses the returned resolver for legacy methods, so migrated and unmigrated methods share one policy implementation.
