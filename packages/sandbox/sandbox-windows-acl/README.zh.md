...
- **直接 API 调用方拥有控制台隔离责任。** 受限子进程若使用 `CREATE_NO_WINDOW` 或 `CREATE_NEW_CONSOLE` 创建，会在 DLL 初始化期间以 `STATUS_DLL_INIT_FAILED`（`0xC0000142`）死亡。Harness 子进程提供方先在独立控制台中启动非受限沙箱 runner，再让受限子进程继承；直接使用 `AclSandbox` 的调用方必须自行提供独立父控制台，否则子进程会共享调用方的控制台。基于管道的 stdio 重定向不受影响。
