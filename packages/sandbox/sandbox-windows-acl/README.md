...
- **Direct API callers own console isolation** — a restricted child created with `CREATE_NO_WINDOW` or `CREATE_NEW_CONSOLE` dies during DLL initialization with `STATUS_DLL_INIT_FAILED` (`0xC0000142`). The Harness subprocess provider starts the unrestricted sandbox runner in a separate console and the restricted child inherits it; direct `AclSandbox` callers must provide their own separate parent console or their child shares the caller's console. Pipe-based stdio redirection is unaffected.
