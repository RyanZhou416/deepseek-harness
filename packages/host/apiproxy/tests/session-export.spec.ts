...
describe('session export compression config', () => {
  it('defaults to level 6 and rejects values outside the integer 0-9 range', () => {
    expect(ApiProxyService.Config({})).toEqual({
      ...
      idleSessionRetentionMs: 300_000,
    })
    expect(ApiProxyService.Config({ sessionExportCompressionLevel: 0 }))
      .toEqual({ sessionExportCompressionLevel: 0, coldBlankProbeMaxBytes: 1024, idleSessionRetentionMs: 300_000 })
    expect(ApiProxyService.Config({ sessionExportCompressionLevel: 9 }))
      .toEqual({ sessionExportCompressionLevel: 9, coldBlankProbeMaxBytes: 1024, idleSessionRetentionMs: 300_000 })
...
describe('cold blank probe config', () => {
  it('accepts a per-Session byte bound including zero and rejects invalid bounds', () => {
    expect(ApiProxyService.Config({ coldBlankProbeMaxBytes: 0 }))
      .toEqual({ sessionExportCompressionLevel: 6, coldBlankProbeMaxBytes: 0, idleSessionRetentionMs: 300_000 })
    expect(ApiProxyService.Config({ coldBlankProbeMaxBytes: 2048 }))
      .toEqual({ sessionExportCompressionLevel: 6, coldBlankProbeMaxBytes: 2048, idleSessionRetentionMs: 300_000 })
...
describe('idle Session retention config', () => {
  it('accepts zero and rejects negative or fractional durations', () => {
    expect(ApiProxyService.Config({ idleSessionRetentionMs: 0 }))
      .toEqual({ sessionExportCompressionLevel: 6, coldBlankProbeMaxBytes: 1024, idleSessionRetentionMs: 0 })
    for (const value of [-1, 1.5]) {
      expect(() => ApiProxyService.Config({ idleSessionRetentionMs: value })).toThrow()
    }
  })
})
