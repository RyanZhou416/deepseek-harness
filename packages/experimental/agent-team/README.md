...
Lead-to-teammate messages steer a live child at its nearest step boundary and wake an inactive child. Messages from teammates retain two peer modes: quiet information does not start an idle member, while a follow-up becomes the recipient's next turn. The sender always sees the outcome — delivered now, or queued. A queued message is already safely stored, so it must not be resent.
...
Lead delivery uses the continuation owner's host-only queue path for an inactive child and the Steer path for a live child, preserving Team provenance while authorizing the Lead-to-child edge. Teammate quiet delivery injects only into a live target; teammate follow-up delivery queues and wakes through the continuation owner. A later follow-up admits earlier quiet mail first so durable mailbox order remains model-visible.
