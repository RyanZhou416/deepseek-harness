...
Lead 发给 teammate 的消息总会在子级最近的 step 边界进行 steering；请求的 quiet 或 follow-up 模式都不能把 Lead 指令放到已排队轮次之后。teammate 发出的消息保留两种 peer 模式：quiet 信息不会启动 idle member，follow-up 则成为接收方的下一个轮次。发送方始终能看到结果——已送达，或正在排队。排队的消息已经安全存储，因此绝不能重发。
