...
- **Send messages** — the Lead's `send_message` and `followup_task` both steer or wake a teammate; teammate callers use `send_message` for quiet information and `followup_task` for work that must start a turn.
...
One stable policy section states the exact Team role/name/id, the explicit-delegation requirement, shared-cwd behavior, filesystem stale-version recovery, Bash/formatter/codegen risk, task and write-scope coordination, quiet/follow-up delivery, the no-retry mailbox rule, and the Lead's duty to wait before answering. The ten Team schemas from `spawn_teammate` through `team_task_update` appear only in Team member scopes.
