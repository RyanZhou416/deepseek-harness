...
describe('Agent Teams profile bundle', () => {
  it('declares a private parseable layer with Team-owned controls', () => {
    ...
    expect(patches.find(patch => patch.id === 'tool-bash')?.config).toMatchObject({ forceRunInBackground: true })
    expect(patches.find(patch => patch.id === 'tool-pwsh')?.config).toMatchObject({ forceRunInBackground: true })
    expect(patches.find(patch => patch.id === 'tool-jobs')?.config).toMatchObject({ yieldWaitOnNextStep: true })
