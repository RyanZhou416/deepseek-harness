...
describe('LocalJobRegistry terminal retention', () => {
  ...
  it('selects the oldest reported finish incrementally when jobs settle out of registration order', async () => {
    vi.useFakeTimers({ now: 1_000 })
    try {
      const ctx = await harness({ maxRetainedTerminalJobsPerOwner: 2 })
      const registeredFirst = producer()
      const firstId = ctx.jobs.start(registeredFirst.spec)
      const finishedFirst = producer()
      const earliestId = ctx.jobs.start(finishedFirst.spec)
      const newest = producer()
      const newestId = ctx.jobs.start(newest.spec)

      finishedFirst.settle({ status: 'completed' })
      await Promise.resolve()
      ctx.jobs.read(earliestId)
      await vi.advanceTimersByTimeAsync(10)
      registeredFirst.settle({ status: 'completed' })
      await Promise.resolve()
      ...
      expect(() => ctx.jobs.get(earliestId)).toThrow(`unknown job ${earliestId}`)
      expect(ctx.jobs.get(firstId).status).toBe('completed')
      expect(ctx.jobs.get(newestId).status).toBe('completed')
  ...
  it('settles, reports, prunes, and expires through indexes without iterating the full store', async () => {
    ...
    const first = producer({ kind: 'subagent' })
    const firstId = ctx.jobs.start(first.spec)
    const second = producer({ kind: 'subagent' })
    const secondId = ctx.jobs.start(second.spec)
    const third = producer({ kind: 'subagent' })
    const thirdId = ctx.jobs.start(third.spec)
    first.settle({ status: 'completed', output: 'first' })
    await Promise.resolve()
    expect(ctx.jobs.read(firstId).text).toBe('first')

    const service = ctx.jobs as unknown as {
      store: Map<JobId, unknown>
      retentionTimer?: ReturnType<typeof setTimeout>
    }
    const firstTimer = service.retentionTimer
    expect(firstTimer).toBeDefined()
    expect(vi.getTimerCount()).toBe(1)
    const values = vi.spyOn(service.store, 'values').mockImplementation(() => {
      throw new Error('retention hot path iterated the complete job store')
    })
    try {
      await vi.advanceTimersByTimeAsync(100)
      second.settle({ status: 'completed', output: 'second' })
      await Promise.resolve()
      expect(service.retentionTimer).toBe(firstTimer)
      expect(() => ctx.jobs.get(firstId)).toThrow(`unknown job ${firstId}`)
      expect(ctx.jobs.read(secondId).text).toBe('second')

      await vi.advanceTimersByTimeAsync(100)
      third.settle({ status: 'completed', output: 'third' })
      await Promise.resolve()
      expect(service.retentionTimer).toBe(firstTimer)
      ...
      expect(ctx.jobs.get(thirdId).status).toBe('completed')
      await vi.advanceTimersByTimeAsync(200)
      expect(() => ctx.jobs.get(thirdId)).toThrow(`unknown job ${thirdId}`)
  ...
  it('shares one timer between deferred count pruning and the next TTL expiry', async () => {
    vi.useFakeTimers({ now: 20_000 })
    try {
      const ctx = await harness({
        terminalJobRetentionMs: 5_000,
        maxRetainedTerminalJobsPerOwner: 1,
      })
      const first = producer()
      const firstId = ctx.jobs.start(first.spec)
