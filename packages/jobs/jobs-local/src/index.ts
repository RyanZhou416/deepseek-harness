...
export class LocalJobRegistry extends JobRegistry {
   ...
   */
  private reportTerminal(job: TrackedJob, deferPrune = false): JobSnapshot {
    job.reported = true
  ...
  private indexTerminal(job: TrackedJob): void {
  ...
  private indexReportedTerminal(job: TrackedJob): void {
  ...
  private refOf(job: TrackedJob): TerminalRef {
    const finishedAt = job.finishedAt
  ...
  private resolveReportedCandidate(owner: Agent | undefined, candidate: TerminalRef): TrackedJob | undefined {
  ...
  private removeTerminal(job: TrackedJob): boolean {
