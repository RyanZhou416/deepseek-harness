...
句柄的方法把带标识的 user 角色消息路由进 agent 的收件箱。`followup()` 排队一条普通的下一个轮次提示词并唤醒驱动器；`steer()` 提交下一步输入并唤醒它；`inject()` 添加面向模型的上下文但不唤醒 idle 驱动器，因此它落在下一个被接纳的步骤中。在运行中的驱动器作出最终 inbox 决定后接纳的输入会在收敛后重放，而不会保持停放状态。`cancel(cause)` 中止当前活动，并在未设置 `keepInbox` 时清除待处理工作；`whenIdle()` 会在整个 agent 达到完全停稳后完成。
