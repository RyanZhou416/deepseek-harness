...
export function apply(ctx: Context): void {
  ctx.tools.register(defineTool({
    ...
    description:
      ...
      + 'terminal, and not a result waiting to be collected; a `send_message` steers the same conversation at its nearest step boundary, '
      + 'and a direct child remains a `send_message` candidate in every status. The snapshot is not a delivery '
