...
An in-process child inherits its parent's Agent preset unless a deployment plugin selects another through `subagent/child-preset` before the child's plugins mount. A missing or unusable proposal logs a warning and leaves inheritance intact. Fresh overrides record `agent-preset/selected`; cold resume uses the logged choice without appending it again. External providers do not use this hook.
...
`applyChildComposition()` runs the `subagent/child-preset` waterfall before joining a preset scope. A listener returns a preset id or calls `next()` to delegate. The helper awaits a nominated mount and records the selected id in the unpublished child's log; an invalid nomination falls back before delegation context and tool restrictions register. A resumed child instead reads its persisted `agentPreset` projection and does not rerun the route selector.
