...
This fork's `standard`, `ptc`, and `cordis` presets include `session_create`, `session_find`, `session_send_message`, and `session_message_status` for separately owned Sessions; `minimal` does not. Creation starts a separate task immediately. Sender attribution comes from the live Agent, and subagent/teammate coordination keeps its own tools.
