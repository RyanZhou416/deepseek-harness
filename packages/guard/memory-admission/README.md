...
This guard bounds process-wide agent-step admission without changing session logs or model input. It acquires a FIFO reservation at `agent/pre-step`, before model-history assembly, and retains that reservation until the matching durable `step/end`; an idle or disposed agent releases a stranded reservation. Active steps finish naturally. New steps wait while either `maxConcurrentSteps` is occupied or V8 heap usage reaches `heapHighWatermarkRatio`, and heap admission reopens only at `heapLowWatermarkRatio`.
...
- An already admitted step may cross the high watermark and is never cancelled by this policy.
