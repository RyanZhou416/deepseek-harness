...
describe('JsonlSessionPersistence: durability and crash semantics', () => {
  ...
  it('shares one metadata scan and its exact revisions across concurrent callers', async () => {
  ...
  it('prunes cached metadata after an artifact disappears', async () => {
    const m = meta('pruned-listed-header')
    await ctx.sessionPersistence.create(m)
    await ctx.sessionPersistence.append(m.id, oneTurnLog())
    const persistence = ctx.sessionPersistence as unknown as {
    ...
    await expect(ctx.sessionPersistence.list()).resolves.toMatchObject([{ id: m.id }])
