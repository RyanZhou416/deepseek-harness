...
Concurrent `list()` callers share one metadata scan but cancel their own wait independently. Validated headers are cached only for the selected physical generation and exact file revision; append, replacement, or removal invalidates that reuse. Returned headers are detached from the cache.
