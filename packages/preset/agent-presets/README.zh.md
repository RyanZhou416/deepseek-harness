...
随附 Web 的 `standard`、`ptc` 与 `cordis` preset 包含[显式文件交付](../../client/ui-deliverables/README.zh.md#explicit-deliveries)和[按 Session 寻址的发现与消息](../../api/tool-session-message/README.zh.md)。`minimal` preset 保留固定的双工具训练配置。
