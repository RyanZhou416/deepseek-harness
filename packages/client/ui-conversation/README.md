...
Automatic composer focus follows the primary pointer. A fine-pointer browser focuses the editor after mount or Session navigation and reveals its restored caret. A coarse-pointer browser leaves the editor blurred so its software keyboard opens only after a direct editor gesture.
