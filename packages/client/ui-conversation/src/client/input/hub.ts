...
export class InputHub implements SessionInputResolver {
   ...
   * button. A turn closing mid-way (`session/*` or `subagent/*`
   * `steer-unavailable`) or a row already claimed (`queue-item-not-found`)
   * converges silently, while a
  ...
  private async steerQueue(session: SessionFace, shell: SessionInputShell): Promise<void> {
    ...
    for (const item of queued) {
      ...
      if (result.error.code === 'session/steer-unavailable'
        ...
        || result.error.code === 'subagent/steer-unavailable'
        || result.error.code === 'subagent/queue-item-not-found') return
