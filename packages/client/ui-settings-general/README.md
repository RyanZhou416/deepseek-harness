...
The same outage, retry, and two-second recovered states appear in `shell.overlay`, so the recovery action remains visible when the sidebar is collapsed or Settings is closed. The global indicator stays silent during healthy startup and uses the same localized labels and reconnect callback as the inline control.
...
The shell is an explicit recovery consumer, so it injects Connection directly rather than adding lifecycle controls to `ctx.remote`. Its private hooks compartment binds `ctx.connection.state`, while the inline control and global overlay receive only the selected state and an injected callback for `ctx.connection.reconnect()`. `ConnectionIndicator` owns both presentations and receives all visible and accessible copy from the `settings` locale namespace; each owner keeps its own two-second recovered-state timer.
