...
Users reach the shell through the sidebar's bottom Settings control; feature plugins contribute their pages and onboarding steps through the slot ledgers this shell projects. After a Host connection failure, a pale-yellow **Disconnected** action appears both beside Settings and at the top-center global overlay, so a collapsed sidebar cannot hide backend loss. Automatic recovery shows **Connecting** with one to three dots advancing every 500ms. Hover or keyboard focus changes either yellow label to **Reconnect now** without changing its background; press feedback stays within the warning palette, and selecting it starts retry 1 immediately. Recovery changes both regions to pale-green **Connected** for two seconds before they disappear. The icon, left-aligned text origin, height, and width remain fixed across every visible state. Initial startup and uninterrupted healthy operation remain silent. The shell renders the modal panel, the navigation built from `settings.section` entries, and exactly one mounted onboarding step at a time.
...
The shell is an explicit recovery consumer, so it injects Connection directly rather than adding lifecycle controls to `ctx.remote`. Its private hooks compartment binds `ctx.connection.state`, while components receive only the selected state and an injected callback for `ctx.connection.reconnect()`. `ConnectionIndicator` owns both inline presentations and receives all visible and accessible copy from the `settings` locale namespace; each presentation owns its two-second recovered-state timer. The global copy registers into `shell.overlay`, outside sidebar clipping and collapse state.
