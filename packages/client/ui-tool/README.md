...
Collapsed rows derive only their summary, state, and body/output presence. Pretty argument text, flattened result output, and validated diff, read, search, or web arrays stay behind cached getters until the disclosure opens, so settled long-session history does not eagerly materialize hidden Tool details.
