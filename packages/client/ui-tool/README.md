...
Generic rows classify known Tool names into search, read, shell, write, edit, code, or generic variants. Running, successful, failed, and interrupted lifecycle states come only from the frozen call/result slice. A collapsed row derives only its title, summary, state, and detail-presence flags; formatted arguments, complete result text, recovery copy, and large card-array copies are cached lazy values first read by the mounted expanded body or details view. File paths resolve against the session `cwd` only when the user invokes the Host open-file callback; presentation code does not read Session services.
