...
通用行把已知工具名称归类为 search、read、shell、write、edit、code 或 generic 变体。运行中、成功、失败和中断状态只来自冻结的 call/result slice。折叠行只派生标题、摘要、状态和详情 presence 标记；格式化参数、完整结果文本、recovery 文案和大型卡片数组副本都是缓存惰性值，只有已挂载的展开 body 或详情视图首次读取时才计算。只有用户调用 Host 打开文件回调时，文件路径才相对会话 `cwd` 解析；展示代码不读取会话服务。
