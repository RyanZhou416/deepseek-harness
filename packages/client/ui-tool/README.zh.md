...
折叠行只派生 summary、state 与 body/output 是否存在。格式化参数文本、压平后的结果输出，以及已验证的 diff、read、search 或 web array 保留在带缓存的 getter 后方，直到 disclosure 打开，因此已结算的长会话历史不会提前物化隐藏的 Tool 详情。
