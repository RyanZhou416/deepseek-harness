...
describe('subagent catalogs', () => {
  it('keeps a catalog-discovered child address across ordinary selection and status frames', async () => {
    ...
    expect(api.callsOf('subagent.history')).toEqual([
      { parentSessionId: S1, childSessionId: S2, mode: 'continuable', maxMessages: 20 },
    ])
...
describe('remaining branches', () => {
  ...
  it('routes stream/error and unknown frames to the documented drops, and dispatches to instantiated sessions', () => {
    ...
    manager.handleMuxEnvelope({ rpcId: 'hb1' as never, payload: { type: 'stream/heartbeat', sentAt: 1 } })
    manager.handleHostEnvelope({ rpcId: 'hb2' as never, payload: { type: 'stream/heartbeat', sentAt: 1 } })
